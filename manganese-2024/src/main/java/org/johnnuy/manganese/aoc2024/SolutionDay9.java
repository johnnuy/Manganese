...
public class SolutionDay9 {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println("Problem 2: %d\n".formatted(timer.time("Problem 2", () -> process(new ClassPathReader("day9/input_1.txt"), SolutionDay9::compactComplete))));
	}
...
//			System.out.println("Current File Id %d had size %d".formatted(f, fileSize));
			
			/* start from our head, and search for a continuous open section */
			int count = 0;
//			System.out.println("Trying to move %d with size %d".formatted(f, fileSize));
			for (int i=0; i<tail; i++) {
				if (list.get(i) == -1) {
					count++;
					/* found a contiguous block, so move the data */
					if (count == fileSize) {
//						System.out.println("Moving %d".formatted(f));
